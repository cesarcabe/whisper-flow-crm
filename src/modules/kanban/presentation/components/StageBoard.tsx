...
export function StageBoard({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Determine target stage - handle both stage-prefixed and raw IDs
    let targetStageId: string | null = null;
    const rawOverId = overId.startsWith('stage-') ? overId.replace('stage-', '') : overId;

    // Check if dropped on a stage
    const overStage = pipeline.stages.find(s => s.id === rawOverId || s.id === overId);
    if (overStage) {
